...
#include "common.h"
#include "com_sun_glass_ui_uia_glass_WinTextRangeProvider.h"
...
GlassTextRangeProvider::GlassTextRangeProvider(JNIEnv* env, jobject jTextRangeProvider, ProxyAccessible* glassAccessible)
: m_refCount(1)
{
  ...
  (JNIEnv *env, jclass jClass)
{
    ...
    mid_Compare = env->GetMethodID(jClass, "Compare", "(Lcom/sun/glass/ui/uia/glass/WinTextRangeProvider;)Z");
    if (env->ExceptionCheck()) return;
    mid_CompareEndpoints = env->GetMethodID(jClass, "CompareEndpoints", "(ILcom/sun/glass/ui/uia/glass/WinTextRangeProvider;I)I");
    if (env->ExceptionCheck()) return;
    ...
    if (env->ExceptionCheck()) return;
    mid_MoveEndpointByRange = env->GetMethodID(jClass, "MoveEndpointByRange", "(ILcom/sun/glass/ui/uia/glass/WinTextRangeProvider;I)V");
    if (env->ExceptionCheck()) return;
...
{
    ProxyAccessible* acc = reinterpret_cast<ProxyAccessible*>(glassAccessible);
