...
(JNIEnv* env, jclass jClass)
{
    ...
    if (env->ExceptionCheck()) return;
    mid_IRawElementProviderSimple_GetPropertyValue = env->GetMethodID(jClass, "IRawElementProviderSimple_GetPropertyValue", "(I)Lat/bestsolution/uia/internal/glass/WinVariant;");
    if (env->ExceptionCheck()) return;
    ...
    if (env->ExceptionCheck()) return;
    // ITextProvider2
    mid_ITextProvider2_GetCaretRange = env->GetMethodID(jClass, "ITextProvider2_GetCaretRange", "()Lat/bestsolution/uia/internal/ProxyAccessible$NCaretRangeResult;");
    if (env->ExceptionCheck()) return;
    ...
    /* NCaretRangeResult */
    jclass jNCaretRangeResultClass = env->FindClass("at/bestsolution/uia/internal/ProxyAccessible$NCaretRangeResult");
    if (env->ExceptionCheck()) return;
...
extern "C" JNIEXPORT jlong JNICALL Java_at_bestsolution_uia_internal_ProxyAccessible_UiaRaiseNotificationEvent
