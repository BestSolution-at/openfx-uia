...
extern "C" JNIEXPORT void JNICALL Java_at_bestsolution_uia_internal_ProxyTextRangeProvider__1initIDs
  (JNIEnv *env, jclass jClass)
{
    mid_Clone = env->GetMethodID(jClass, "Clone", "()J");
    if (env->ExceptionCheck()) return;
    mid_Compare = env->GetMethodID(jClass, "Compare", "(Lat/bestsolution/uia/internal/ProxyTextRangeProvider;)Z");
    if (env->ExceptionCheck()) return;
    mid_CompareEndpoints = env->GetMethodID(jClass, "CompareEndpoints", "(ILat/bestsolution/uia/internal/ProxyTextRangeProvider;I)I");
    if (env->ExceptionCheck()) return;
    mid_ExpandToEnclosingUnit = env->GetMethodID(jClass, "ExpandToEnclosingUnit", "(I)V");
    if (env->ExceptionCheck()) return;
    mid_FindAttribute = env->GetMethodID(jClass, "FindAttribute", "(ILat/bestsolution/uia/internal/glass/WinVariant;Z)J");
    if (env->ExceptionCheck()) return;
    ...
    if (env->ExceptionCheck()) return;
    mid_GetAttributeValue = env->GetMethodID(jClass, "GetAttributeValue", "(I)Lat/bestsolution/uia/internal/glass/WinVariant;");
    if (env->ExceptionCheck()) return;
    ...
    if (env->ExceptionCheck()) return;
    mid_MoveEndpointByRange = env->GetMethodID(jClass, "MoveEndpointByRange", "(ILat/bestsolution/uia/internal/ProxyTextRangeProvider;I)V");
    if (env->ExceptionCheck()) return;
...
extern "C" JNIEXPORT jlong JNICALL Java_at_bestsolution_uia_internal_ProxyTextRangeProvider__1createTextRangeProvider
...
extern "C" JNIEXPORT void JNICALL Java_at_bestsolution_uia_internal_ProxyTextRangeProvider__1destroyTextRangeProvider
