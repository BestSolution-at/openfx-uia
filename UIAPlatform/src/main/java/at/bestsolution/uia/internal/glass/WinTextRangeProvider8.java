...
public class WinTextRangeProvider8 implements IWinTextRangeProvider {

  private static Logger LOG = LoggerFactory.create(WinTextRangeProvider8.class);
    ...
    WinTextRangeProvider8(WinAccessible8 accessible) {
    ...
    public IWinTextRangeProvider Clone() {
        WinTextRangeProvider8 clone = new WinTextRangeProvider8(accessible);
    ...
    public IWinTextRangeProvider FindText(String text, boolean backward, boolean ignoreCase) {
        ...
        WinTextRangeProvider8 result = new WinTextRangeProvider8(accessible);
...
//        System.out.println("+GetText [" + text.substring(start, endOffset)+"]");
    ...
    public void MoveEndpointByRange(int endpoint, IWinTextRangeProvider targetRange, int targetEndpoint) {
        WinTextRangeProvider8 target = (WinTextRangeProvider8) targetRange;
        String text = (String)getAttribute(TEXT);
        if (text == null) return;
        int length = text.length();

        int offset = targetEndpoint == TextPatternRangeEndpoint_Start ? target.start : target.end;
        if (endpoint == TextPatternRangeEndpoint_Start) {
            start = offset;
        } else {
            end = offset;
